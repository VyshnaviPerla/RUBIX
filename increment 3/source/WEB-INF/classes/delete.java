...
DateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	java.util.Date dd,a7;
        ...
        { 
         ResultSet rts=s1.executeQuery("select * from inbox where mailid="+id[i]);
...
a7= (java.util.Date)formatter.parse(rts.getString("dateofdelivery"));
	   ...
	   sq="insert into trashcan values('"+a2+"','"+a1+"','"+a3+"','"+form.format(dd)+"','"+a4+"','"+a5+"',"+id[i]+",to_date('"+form.format(a6)+"','DD.MM.YYYY:HH24:MI:SS'),'"+form.format(a7)+"')";
           ...
           s1.executeUpdate(sq);
            ...
            j=s.executeUpdate("delete from inbox where mailid="+id[i]);
         }
        
       if(j!=0)
       {
            rd= rq.getRequestDispatcher("welcomebodyd.html");
                               rd.forward(rq,rs); 
       }
        pw.close();
    }
